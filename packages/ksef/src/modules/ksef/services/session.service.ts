...
export class KsefSessionService {
  ...
  ): Promise<KsefSession | null> {
    return em.findOne(KsefSession, {
  ...
  async getSessionById(em: EntityManager, sessionId: string): Promise<KsefSession | null> {
    return em.findOne(KsefSession, { id: sessionId })
  }
  ...
  async closeStaleActiveSessions(
    ...
    const staleSessions = await em.find(KsefSession, {
      tenantId: params.tenantId,
      organizationId: params.organizationId,
      sessionStatus: 'active',
      startedAt: { $lt: cutoff },
