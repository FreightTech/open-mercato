...
export class Migration20260209231639 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create index if not exists "fms_invoice_line_items_product_idx" on "fms_invoice_line_items" ("product_id");`);
    ...
    this.addSql(`do $$ begin alter table "fms_invoice_pages" add constraint "fms_invoice_pages_invoice_id_foreign" foreign key ("invoice_id") references "fms_invoices" ("id") on update cascade on delete cascade; exception when duplicate_object then null; end $$;`);
