...
export default function AllocateCostsPage({ params }: RouteParams) {
  ...
  const { data: projectsData, isLoading: projectsLoading } = useQuery({
    queryKey: ['invoice-project-lines', invoiceId],
    queryFn: async (): Promise<{ projects: MatchedProject[] }> => {
      const { result } = await apiCall(`/api/fms_documents/invoices/${invoiceId}/project-lines`)
  ...
  const { data: allocationsData } = useQuery({
    ...
    queryFn: async (): Promise<{ allocations: Allocation[] }> => {
      const { result } = await apiCall(`/api/fms_documents/invoices/${invoiceId}/allocations`)
  ...
  // Build a lookup: lineItemId -> saved allocation
  const savedAllocationMap = useMemo(() => {
    const map = new Map<string, Allocation>()
    for (const a of savedAllocations) {
      map.set(a.invoiceLineItemId, a)
    }
    return map
  }, [savedAllocations])

  const allocatedCount = savedAllocations.length + pendingAllocations.size

  const handleLineItemClick = useCallback((lineItemId: string) => {
    // Don't allow re-assigning saved allocations
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const allocations = Array.from(pendingAllocations.entries()).map(([lineItemId, target]) => {
        const lineItem = activeLineItems.find((li) => li.id === lineItemId)
        return {
          lineItemId,
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['invoice-project-lines', invoiceId] })
    },
  })
      ...
      <div className="flex-shrink-0 border-b bg-background">
        <div className="flex items-center px-6 py-0">
          <button
            ...
            onClick={() => router.push(`/backend/fms-documents/invoices/${invoiceId}`)}
          >
            1. Verify invoice
          </button>
          <button
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {selectedProjectLineId && (
              <div className="text-sm text-primary mb-4 p-3 bg-primary/5 rounded-lg border border-primary/10">
                Click cost lines to assign them to the selected offer line.
              </div>
            )}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground mt-0.5">
                          Qty: {li.quantity} &middot; {formatCurrency(li.grossAmount, invoice.currencyCode)}
                        </div>
                      </div>
