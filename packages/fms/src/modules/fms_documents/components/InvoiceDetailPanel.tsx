...
export function InvoiceDetailPanel({
  ...
  const autoMatchMutation = useMutation({
    mutationFn: async () => {
      const { result } = await apiCall(`/api/fms_documents/invoices/${invoiceId}/match-charges`, {
        method: 'POST',
  ...
  const updateLineItemMutation = useMutation({
    mutationFn: async (item: Partial<LineItem> & { id: string }) => {
      const { result } = await apiCall(`/api/fms_documents/invoices/${invoiceId}/line-items/${item.id}`, {
        method: 'PATCH',
  ...
  const createLineItemMutation = useMutation({
    mutationFn: async (item: Partial<LineItem>) => {
      const { result } = await apiCall(`/api/fms_documents/invoices/${invoiceId}/line-items`, {
        method: 'POST',
        body: JSON.stringify(item),
      })
      return result
