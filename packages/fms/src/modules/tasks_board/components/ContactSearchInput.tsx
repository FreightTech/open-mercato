...
function formatContactName(item: ContactItem): string {
...
export function ContactSearchInput({
  ...
  const [cachedItem, setCachedItem] = useState<ContactItem | null>(null)
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['task-board-contacts', debouncedSearch, contractorId],
    queryFn: async () => {
      const params = new URLSearchParams()
      if (debouncedSearch) params.set('search', debouncedSearch)
      params.set('pageSize', '50')
      if (contractorId) params.set('contractorId', contractorId)
      const result = await apiCall<{ items?: ContactItem[] }>(
        `/api/contractors/contacts?${params.toString()}`,
      )
  ...
  const handleSelect = useCallback(
    (item: ContactItem) => {
      setCachedItem(item)
      onChange(item.id, formatContactName(item))
      setSearchQuery('')
      setIsFocused(false)
  ...
  const handleFocus = useCallback(() => {
    setIsFocused(true)
    if (selectedItem) {
      setSearchQuery(formatContactName(selectedItem))
  ...
  const displayValue = isFocused ? searchQuery : (selectedItem ? formatContactName(selectedItem) : '')
          ...
          <div ref={listRef} style={{ maxHeight: '220px', overflowY: 'auto' }}>
            {isLoading ? (
              ...
              options.map((item, index) => {
                ...
                const fullName = formatContactName(item)
                return (
                  <button
                    key={item.id}
                    type="button"
