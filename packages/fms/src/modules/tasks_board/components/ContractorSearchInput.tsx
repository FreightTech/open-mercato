...
type ContractorSearchInputProps = {
  value: string | null
...
export function ContractorSearchInput({
  value,
  onChange,
  placeholder = 'Company...',
  disabled = false,
}: ContractorSearchInputProps) {
  const [isFocused, setIsFocused] = useState(false)
  const [searchQuery, setSearchQuery] = useState('')
  const [debouncedSearch, setDebouncedSearch] = useState('')
  const [highlightedIndex, setHighlightedIndex] = useState(0)
  const [cachedItem, setCachedItem] = useState<ContractorItem | null>(null)
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['task-board-contractors', debouncedSearch],
    queryFn: async () => {
      const params = new URLSearchParams()
      if (debouncedSearch) params.set('search', debouncedSearch)
      params.set('pageSize', '50')
      params.set('isActive', 'true')
      const result = await apiCall<{ items?: ContractorItem[] }>(
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                      <Building2 style={{ width: 12, height: 12, flexShrink: 0, opacity: 0.4 }} />
