// src/modules/transports/vector.ts
    ...
    entities: [
        {
            ...
            buildSource: async (ctx) => {
                ...
                console.log('[transports] lines:', lines)
                ...
                return {
                    input: lines,
                    presenter: {
                        title: r.tracking_number || 'Transport',
                        subtitle,
                        icon: 'truck'
                    },
                    checksumSource: {
            ...
            formatResult: async ({ record }) => ({
                title: record.tracking_number || 'Transport',
                subtitle: `${record.status} • ${record.origin_city} → ${record.destination_city}`,
                icon: 'truck'
            }),
            resolveUrl: async ({ record }) => `/backend/transports?id=${record.id}`
        }
    ]
}
