...
import { createPageImageService } from '../../../services/page-image.service'
...
const routeMetadata = {
  POST: { requireAuth: true, requireFeatures: ['fms_financials.invoices.upload'] },
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { result } = await bus.execute('fms_financials.invoices.create', {
      input: {
    ...
    if (file.type === 'application/pdf') {
      try {
        const pageImageService = createPageImageService()
        ...
        console.error('[fms_financials] Page extraction failed:', pageErr)
      }
    }
