...
function resolveSchemasDir(): string {
  ...
  const possiblePaths = [
    ...
    path.join(process.cwd(), 'packages/fms/src/modules/fms_financials/data/schemas'),
    ...
    path.join(path.dirname(fileURLToPath(import.meta.url)), '..', '..', '..', '..', 'src', 'modules', 'fms_financials', 'data', 'schemas'),
  ]
