...
const crud = makeCrudRoute({
  ...
  orm: {
    entity: FmsInvoicingInvoice,
    idField: 'id',
    orgField: 'organizationId',
    tenantField: 'tenantId',
    softDeleteField: 'deletedAt',
  },
  indexer: { entityType: 'invoicing:fms_invoicing_invoice' },
  list: {
    schema: listSchema,
    entityId: 'invoicing:fms_invoicing_invoice',
    fields: [
      'id',
      'invoice_number',
      'invoice_date',
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { result } = await bus.execute('fms_invoicing.invoices.create', {
      input: {
        ...parse.data,
