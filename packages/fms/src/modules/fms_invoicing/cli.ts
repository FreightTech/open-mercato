...
const setupTestCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!nip) {
      console.log(`
Usage: yarn mercato fms_invoicing ksef:setup-test --nip <NIP> [options]
...
Examples:
  yarn mercato fms_invoicing ksef:setup-test --nip 7980332920 --pesel 30112206276
  yarn mercato fms_invoicing ksef:setup-test --nip 7980332920 --desc "My Test Corp"
`)
      return
    }
    ...
    console.log(`     yarn mercato fms_invoicing ksef:add-credential --nip ${nip} --cert ksef-test-cert.pem --key ksef-test-key.pem --tenant <id> --org <id>`)
  },
}
...
const cleanupTestCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!nip) {
      console.error('Usage: yarn mercato fms_invoicing ksef:cleanup-test --nip <NIP> [--pesel <PESEL>] [--env test]')
      return
    }
...
const addCredentialCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!nip || !tenantId || !organizationId) {
      console.log(`
Usage: yarn mercato fms_invoicing ksef:add-credential --nip <NIP> --tenant <id> --org <id> [options]
...
const listCredentialsCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: yarn mercato fms_invoicing ksef:list-credentials --tenant <id> --org <id>')
      return
    }
    ...
    try {
      const credentials = await em.find(FmsInvoicingKsefCredential, {
        tenantId,
        organizationId,
      })
...
const statusCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: yarn mercato fms_invoicing ksef:status --tenant <id> --org <id>')
      return
    }
    ...
    try {
      ...
      const credentialCount = await em.count(FmsInvoicingKsefCredential, { tenantId, organizationId })
      const activeSessionCount = await em.count(FmsInvoicingKsefSession, {
        tenantId,
        organizationId,
      ...
      const invoiceCounts = {
        total: await em.count(FmsInvoicingInvoice, { tenantId, organizationId, deletedAt: null }),
        draft: await em.count(FmsInvoicingInvoice, { tenantId, organizationId, status: 'draft', deletedAt: null }),
        approved: await em.count(FmsInvoicingInvoice, { tenantId, organizationId, status: 'approved', deletedAt: null }),
        ksefQueued: await em.count(FmsInvoicingInvoice, { tenantId, organizationId, ksefStatus: 'queued', deletedAt: null }),
        ksefSubmitted: await em.count(FmsInvoicingInvoice, { tenantId, organizationId, ksefStatus: 'submitted', deletedAt: null }),
        ksefAccepted: await em.count(FmsInvoicingInvoice, { tenantId, organizationId, ksefStatus: 'accepted', deletedAt: null }),
        ksefRejected: await em.count(FmsInvoicingInvoice, { tenantId, organizationId, ksefStatus: 'rejected', deletedAt: null }),
        ksefError: await em.count(FmsInvoicingInvoice, { tenantId, organizationId, ksefStatus: 'error', deletedAt: null }),
      }

      console.log('\nFMS Invoicing Status')
...
// ========================================
// ksef:backfill — Import existing FmsInvoices
// ========================================
...
const backfillCommand: ModuleCli = {
  ...
  async run(rest) {
    ...
    if (!tenantId || !organizationId) {
      console.error('Usage: yarn mercato fms_invoicing ksef:backfill --tenant <id> --org <id> [--status approved]')
      return
    }
    ...
    try {
      ...
      const existingIds = await knex.raw<{ rows: Array<{ source_document_invoice_id: string }> }>(
        `SELECT source_document_invoice_id FROM fms_invoicing_invoices
...
const helpCommand: ModuleCli = {
  ...
  async run() {
    console.log(`
Usage: yarn mercato fms_invoicing <command> [options]
...
Status & Operations:
  ...
  ksef:backfill           Import existing FmsInvoices from fms_documents
...
Examples:
  ...
  yarn mercato fms_invoicing ksef:add-credential --nip 7980332920 --cert cert.pem --key key.pem --tenant <id> --org <id>
  yarn mercato fms_invoicing ksef:status --tenant <id> --org <id>
  yarn mercato fms_invoicing ksef:backfill --tenant <id> --org <id>
`)
  },
}
