...
import type { FmsInvoicingInvoiceSnapshot } from '../data/snapshots'
...
const queueKsefCommand: CommandHandler<QueueKsefInput, { id: string }> = {
  ...
  async execute(rawInput, ctx) {
    ...
    await emitFmsInvoicingEvent('fms_invoicing.ksef.queued', {
      id: record.id,
      tenantId: record.tenantId,
      organizationId: record.organizationId,
...
const updateKsefStatusCommand: CommandHandler<UpdateKsefStatusInput, { id: string }> = {
  id: 'fms_invoicing.ksef.update_status',
  async execute(rawInput, ctx) {
    ...
    const ksefEventMap: Record<string, string> = {
      submitted: 'fms_invoicing.ksef.submitted',
      accepted: 'fms_invoicing.ksef.accepted',
      rejected: 'fms_invoicing.ksef.rejected',
      error: 'fms_invoicing.ksef.error',
      upo_downloaded: 'fms_invoicing.ksef.upo_downloaded',
    }

    const eventId = ksefEventMap[input.ksefStatus]
    if (eventId) {
      await emitFmsInvoicingEvent(eventId as Parameters<typeof emitFmsInvoicingEvent>[0], {
        id: record.id,
        tenantId: record.tenantId,
        organizationId: record.organizationId,
        invoiceNumber: record.invoiceNumber,
