...
import { z } from 'zod'
import { FmsInvoicingInvoice, FmsInvoicingLineItem, FmsInvoicingSettings } from '../data/entities'
import type { FmsInvoicingInvoiceSnapshot, InvoiceUndoPayload } from '../data/snapshots'
import {
  createInvoiceSchema,
  updateInvoiceSchema,
...
const createInvoiceCommand: CommandHandler<CreateInvoiceInput, { id: string }> = {
  id: 'fms_invoicing.invoices.create',
  async execute(rawInput, ctx) {
    ...
    if (isCorrection) {
      ...
      const correctedInvoice = await em.findOne(FmsInvoicingInvoice, {
  ...
  undo: async ({ logEntry, ctx }) => {
    ...
    await em.nativeDelete(FmsInvoicingLineItem, { invoice: invoiceId })

    // Delete invoice
    const invoice = await em.findOne(FmsInvoicingInvoice, { id: invoiceId })
    if (!invoice) return
...
const updateInvoiceCommand: CommandHandler<UpdateInvoiceInput, { id: string }> = {
  id: 'fms_invoicing.invoices.update',
  ...
  async execute(rawInput, ctx) {
    ...
    const invoice = await em.findOne(FmsInvoicingInvoice, { id: input.id, deletedAt: null })
    ...
    if (input.lineItems !== undefined) {
      await em.nativeDelete(FmsInvoicingLineItem, { invoice: record.id })
  ...
  buildLog: async ({ ctx, snapshots, result }) => {
    const before = snapshots.before as FmsInvoicingInvoiceSnapshot | undefined
  ...
  undo: async ({ logEntry, ctx }) => {
    ...
    const de = ctx.container.resolve('dataEngine') as DataEngine
    const invoice = await em.findOne(FmsInvoicingInvoice, { id: before.id })
    if (invoice) {
...
const deleteInvoiceCommand: CommandHandler<{ id?: string; body?: Record<string, unknown>; query?: Record<string, unknown> }, { id: string }> = {
  id: 'fms_invoicing.invoices.delete',
  ...
  async execute(input, ctx) {
    ...
    const invoice = await em.findOne(FmsInvoicingInvoice, { id, deletedAt: null })
    const record = assertRecordFound(invoice, 'Invoice not found')
    ensureTenantScope(ctx, record.tenantId)
    ensureOrganizationScope(ctx, record.organizationId)
...
const approveInvoiceCommand: CommandHandler<ApproveInvoiceInput, { id: string }> = {
  id: 'fms_invoicing.invoices.approve',
  async prepare(rawInput, ctx) {
    const em = ctx.container.resolve('em') as EntityManager
  ...
  async execute(rawInput, ctx) {
    ...
    await emitFmsInvoicingEvent('fms_invoicing.invoice.approved', {
      id: record.id,
      tenantId: record.tenantId,
      organizationId: record.organizationId,
...
const rejectInvoiceCommand: CommandHandler<RejectInvoiceInput, { id: string }> = {
  id: 'fms_invoicing.invoices.reject',
  async prepare(rawInput, ctx) {
    const em = ctx.container.resolve('em') as EntityManager
    const snapshot = await loadInvoiceSnapshot(em, rawInput.id)
    return snapshot ? { before: snapshot } : {}
