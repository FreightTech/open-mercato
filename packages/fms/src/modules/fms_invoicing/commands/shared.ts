...
import type {
  FmsInvoicingInvoiceSnapshot,
  FmsInvoicingLineItemSnapshot,
} from '../data/snapshots'
 ...
 */
function serializeLineItemSnapshot(lineItem: FmsInvoicingLineItem): FmsInvoicingLineItemSnapshot {
  return {
    id: lineItem.id,
...
export async function loadInvoiceSnapshot(
  em: EntityManager,
  invoiceId: string
): Promise<FmsInvoicingInvoiceSnapshot | null> {
  ...
  const lineItemSnapshots: FmsInvoicingLineItemSnapshot[] = lineItems.map(serializeLineItemSnapshot)
...
export async function applyInvoiceSnapshot(
  em: EntityManager,
  snapshot: FmsInvoicingInvoiceSnapshot
): Promise<FmsInvoicingInvoice> {
  let invoice = await em.findOne(FmsInvoicingInvoice, { id: snapshot.id })

  if (!invoice) {
    invoice = em.create(FmsInvoicingInvoice, {
      id: snapshot.id,
...
export async function applyLineItemSnapshot(
  em: EntityManager,
  snapshot: FmsInvoicingLineItemSnapshot
): Promise<FmsInvoicingLineItem> {
  let lineItem = await em.findOne(FmsInvoicingLineItem, { id: snapshot.id })

  if (!lineItem) {
    lineItem = em.create(FmsInvoicingLineItem, {
      id: snapshot.id,
      organizationId: snapshot.organizationId,
      tenantId: snapshot.tenantId,
      invoice: em.getReference(FmsInvoicingInvoice, snapshot.invoiceId),
