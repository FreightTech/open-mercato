...
export function useInvoiceBuilderState(editId?: string | null) {
  ...
  const loadSellerDefaults = useCallback(async () => {
    ...
    try {
      const res = await apiCall('/api/fms_invoicing/settings')
      if (!res.ok) return
  ...
  const loadInvoice = useCallback(async (id: string) => {
    setLoading(true)
    try {
      const res = await apiCall(`/api/fms_invoicing/invoices/${id}`)
  ...
  const save = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      if (invoiceId) {
        const res = await apiCall(`/api/fms_invoicing/invoices/${invoiceId}`, {
          method: 'PATCH',
        ...
        const res = await apiCall('/api/fms_invoicing/invoices', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            ...payload,
  ...
  const generatePreview = useCallback(async () => {
    ...
    setPdfError(null)
    try {
      const response = await fetch('/api/fms_invoicing/invoices/preview-pdf', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const loadPdfPreview = useCallback(async (id?: string) => {
    ...
    setPdfError(null)
    try {
      const response = await fetch(`/api/fms_invoicing/invoices/${targetId}/pdf`)
      if (!response.ok) throw new Error(`HTTP ${response.status}`)
      const blob = await response.blob()
      revokePreviousBlob()
      const url = URL.createObjectURL(blob)
