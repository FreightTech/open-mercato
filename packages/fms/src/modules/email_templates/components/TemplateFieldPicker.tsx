...
export function TemplateFieldPicker({ templateType, onInsertTag, disabled }: Props) {
  ...
  const allFields = React.useMemo(() => 
    [...commonFields, ...templateFields].sort((a, b) => {
      const labelA = t(`email_templates.fields.labels.${a.tag}`, a.tag)
      const labelB = t(`email_templates.fields.labels.${b.tag}`, b.tag)
  ...
  const getFieldLabel = (tag: string) => {
    return t(`email_templates.fields.labels.${tag}`, tag)
  }
  
  const getFieldDescription = (tag: string) => {
    return t(`email_templates.fields.descriptions.${tag}`, '')
  }
  
  const getControlLabel = (id: string) => {
    return t(`email_templates.fields.labels.${id}`, id)
  }
  
  const getControlDescription = (id: string) => {
    return t(`email_templates.fields.descriptions.${id}`, '')
  }
    ...
    <div className="flex flex-col space-y-4">
      <div>
        <h4 className="text-sm font-semibold mb-3">
          {t('email_templates.fields.header', 'Available Fields')}
        </h4>
        <TooltipProvider>
      ...
      <div className="border-t pt-4">
        <h4 className="text-sm font-semibold mb-3">
          {t('email_templates.fields.advanced_header', 'Advanced')}
        </h4>
        <TooltipProvider>
