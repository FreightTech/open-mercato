...
type DraftQuotesData = {
  count: number
  maxLagMs: number | null
  maxLagQuoteId: string | null
...
async function loadDraftQuotes(): Promise<DraftQuotesData> {
  const call = await apiCall<DraftQuotesData>('/api/fms_quotes/dashboard/widgets/draft-quotes')
  if (!call.ok) {
    const message =
...
const LAG_THRESHOLD_MS = 48 * 60 * 60 * 1000 // 48 hours
const LAG_WARNING_MS = 24 * 60 * 60 * 1000 // 24 hours

const DraftQuotesWidget: React.FC<DashboardWidgetComponentProps<DraftQuotesSettings>> = ({
  mode,
  ...
  const t = useT()
  const hydrated = React.useMemo(() => hydrateDraftQuotesSettings(settings), [settings])
  const [data, setData] = React.useState<DraftQuotesData | null>(null)
  ...
  const refresh = React.useCallback(async () => {
    ...
    setError(null)
    try {
      const result = await loadDraftQuotes()
      setData(result)
    } catch (err) {
      console.error('Failed to load draft quotes widget data', err)
      setError(t('fms_quotes.widgets.draftQuotes.error'))
    } finally {
      setLoading(false)
        ...
        <div className="flex items-center gap-2 border-r pr-3">
          ...
          <div className="flex flex-col gap-0.5 text-xs">
            <div className="text-muted-foreground leading-tight">
              {data.count === 1 ? 'draft' : 'draft'}
              <br />
              {data.count === 1 ? 'quote' : 'quotes'}
            </div>
            {/* Trend */}
              ...
              {lagLevel !== 'normal' && (
                <div className={`flex items-center gap-1 ${countColor}`}>
                  ...
                  <span className="font-medium">
                    {lagLevel === 'critical' ? 'Urgent' : 'Review soon'}
                  </span>
                </div>
              )}
            </>
          ) : (
            <div className="text-muted-foreground">No pending drafts</div>
          )}
...
export default DraftQuotesWidget
