...
export class Migration20260202184614 extends Migration {
  ...
  override async down(): Promise<void> {
    this.addSql(`alter table "fms_offers" add column if not exists "contract_type" text not null default 'spot', add column if not exists "carrier_name" text null, add column if not exists "currency_code" text not null default 'USD', add column if not exists "total_amount" numeric(18,4) not null default '0';`);

    this.addSql(`alter table "fms_offer_lines" drop column if exists "carrier_id";`);

    this.addSql(`alter table "fms_offer_lines" add column if not exists "product_type" text null, add column if not exists "provider_name" text null;`);
