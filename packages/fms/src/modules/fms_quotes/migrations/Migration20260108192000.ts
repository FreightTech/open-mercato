...
export class Migration20260108192000 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "fms_quotes" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "quote_number" text not null, "status" text not null default 'draft', "direction" text not null, "incoterm" text null, "cargo_type" text not null, "origin_port_code" text null, "destination_port_code" text null, "valid_until" timestamptz null, "currency_code" text not null default 'USD', "notes" text null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, constraint "fms_quotes_pkey" primary key ("id"));`);
    ...
    this.addSql(`
      do $$ begin
        alter table "fms_offers" add constraint "fms_offers_quote_id_foreign" foreign key ("quote_id") references "fms_quotes" ("id") on update cascade;
      exception when others then null; end $$;
    `);
    this.addSql(`
      do $$ begin
        alter table "fms_offer_lines" add constraint "fms_offer_lines_offer_id_foreign" foreign key ("offer_id") references "fms_offers" ("id") on update cascade;
      exception when others then null; end $$;
    `);
  }
    ...
    this.addSql(`
      do $$ begin
        if exists (select 1 from information_schema.tables where table_name = 'fms_offers') then
          alter table "fms_offers" drop constraint if exists "fms_offers_quote_id_foreign";
        end if;
      end $$;
    `);
    this.addSql(`
      do $$ begin
        if exists (select 1 from information_schema.tables where table_name = 'fms_offer_lines') then
          alter table "fms_offer_lines" drop constraint if exists "fms_offer_lines_offer_id_foreign";
        end if;
      end $$;
    `);
