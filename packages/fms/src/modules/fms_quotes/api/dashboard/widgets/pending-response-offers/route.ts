...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Get current pending response offers with quote for currency
    ...
    console.error('fms_quotes.widgets.pendingResponseOffers failed', err)
    return NextResponse.json(
      { error: translate('fms_quotes.widgets.pendingResponseOffers.error', 'Failed to load pending response offers data') },
      { status: 500 },
    )
  }
}
