...
export class Migration20260108145253_contractors extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table if not exists "contractors" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "name" text not null, "short_name" text null, "code" text null, "parent_id" uuid null, "tax_id" text null, "legal_name" text null, "registration_number" text null, "is_active" boolean not null default true, "created_at" timestamptz not null, "updated_at" timestamptz not null, "deleted_at" timestamptz null, constraint "contractors_pkey" primary key ("id"));`);
    this.addSql(`create index if not exists "contractors_parent_idx" on "contractors" ("parent_id");`);
    this.addSql(`create index if not exists "idx_contractors_tenant_org_id" on "contractors" ("tenant_id", "organization_id", "id") where deleted_at is null;`);
    ...
    this.addSql(`create table if not exists "contractor_addresses" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "purpose" text not null, "label" text null, "address_line1" text not null, "address_line2" text null, "city" text not null, "state" text null, "postal_code" text null, "country_code" text not null, "is_primary" boolean not null default false, "is_active" boolean not null default true, "created_at" timestamptz not null, "updated_at" timestamptz not null, "contractor_id" uuid not null, constraint "contractor_addresses_pkey" primary key ("id"));`);
    ...
    this.addSql(`create index if not exists "idx_contractor_role_types_category" on "contractor_role_types" ("tenant_id", "organization_id", "category") where is_active = true;`);
    ...
    this.addSql(`do $$ begin alter table "contractor_role_types" add constraint "contractor_role_types_code_unique" unique ("organization_id", "tenant_id", "code"); exception when others then null; end $$;`);
    ...
    this.addSql(`do $$ begin alter table "contractor_addresses" add constraint "contractor_addresses_contractor_id_foreign" foreign key ("contractor_id") references "contractors" ("id") on update cascade; exception when others then null; end $$;`);

    this.addSql(`do $$ begin alter table "contractor_contacts" add constraint "contractor_contacts_contractor_id_foreign" foreign key ("contractor_id") references "contractors" ("id") on update cascade; exception when others then null; end $$;`);

    this.addSql(`do $$ begin alter table "contractor_credit_limits" add constraint "contractor_credit_limits_contractor_id_foreign" foreign key ("contractor_id") references "contractors" ("id") on update cascade; exception when others then null; end $$;`);

    this.addSql(`do $$ begin alter table "contractor_payment_terms" add constraint "contractor_payment_terms_contractor_id_foreign" foreign key ("contractor_id") references "contractors" ("id") on update cascade; exception when others then null; end $$;`);
