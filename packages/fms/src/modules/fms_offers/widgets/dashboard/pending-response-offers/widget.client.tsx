...
async function loadPendingResponseOffers(): Promise<PendingResponseOffersData> {
  const call = await apiCall<PendingResponseOffersData>('/api/fms_offers/dashboard/widgets/pending-response-offers')
  if (!call.ok) {
    const message =
...
const PendingResponseOffersWidget: React.FC<DashboardWidgetComponentProps<PendingResponseOffersSettings>> = ({
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      setError(t('fms_offers.widgets.pendingResponseOffers.error'))
    } finally {
      setLoading(false)
