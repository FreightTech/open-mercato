...
export async function resolveWidgetScope(
  ...
  if (!auth) {
    throw new CrudHttpError(401, { error: translate('fms_offers.errors.unauthorized', 'Unauthorized') })
  }
  ...
  if (!tenantId) {
    throw new CrudHttpError(400, { error: translate('fms_offers.errors.tenant_required', 'Tenant context is required') })
  }
  ...
  if (organizationIds !== null && organizationIds.length === 0) {
    throw new CrudHttpError(400, { error: translate('fms_offers.errors.organization_required', 'Organization context is required') })
  }
