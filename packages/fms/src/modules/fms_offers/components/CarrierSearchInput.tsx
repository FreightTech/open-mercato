...
export function CarrierSearchInput({
  value,
  onChange,
  placeholder = 'Carrier...',
  disabled = false,
}: CarrierSearchInputProps) {
  const [isFocused, setIsFocused] = useState(false)
  const [searchQuery, setSearchQuery] = useState('')
  const [debouncedSearch, setDebouncedSearch] = useState('')
  const [highlightedIndex, setHighlightedIndex] = useState(0)
  const [cachedItem, setCachedItem] = useState<CarrierItem | null>(null)
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['fms-carriers-search', debouncedSearch],
    queryFn: async () => {
      const params = new URLSearchParams()
      if (debouncedSearch) params.set('q', debouncedSearch)
      params.set('limit', '50')
      params.set('isActive', 'true')
      const result = await apiCall<{ items?: CarrierItem[] }>(
        `/api/fms_products/carriers?${params.toString()}`,
      )
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', flex: 1, minWidth: 0 }}>
      ...
      >
        <Truck style={{ width: 14, height: 14, flexShrink: 0, opacity: 0.4 }} />
        <input
          ref={inputRef}
          type="text"
          value={displayValue}
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                      <Truck style={{ width: 12, height: 12, flexShrink: 0, opacity: 0.4 }} />
