...
type RfqActivitySectionProps = {
  rfqId: string
}

export function RfqActivitySection({ rfqId }: RfqActivitySectionProps) {
  const [filter, setFilter] = useState<ActivityFilter>('all')
  const queryClient = useQueryClient()

  const { data, isLoading } = useQuery({
    queryKey: ['rfq_activity', rfqId, filter],
    queryFn: async () => {
      const response = await apiCall<{ items: ActivityEntry[]; total: number }>(
        `/api/fms_offers/rfq/${rfqId}/activity?filter=${filter}`
      )
