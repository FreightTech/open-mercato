...
export function OfferDialog({
  ...
  const handleSubmit = React.useCallback(async () => {
    ...
    try {
      ...
      let response
      if (mode === 'create') {
        response = await apiCall('/api/fms_offers/offers', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        })
      } else {
        response = await apiCall(`/api/fms_offers/offers/${initialValues?.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payload),
        })
  ...
  }, [formData, rfqId, mode, initialValues?.id, onOpenChange, onSuccess])
      ...
      <DialogContent className="sm:max-w-lg">
        <div onKeyDown={handleKeyDown}>
          <DialogHeader>
            ...
            <DialogDescription>
              {mode === 'create'
                ? 'Create a new offer for this RFQ.'
