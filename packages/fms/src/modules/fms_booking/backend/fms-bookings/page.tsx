/**
 * FMS Booking Module - List View
 * Bookings list with DynamicTable
 */
...
export default function BookingsListPage() {
  ...
  const { data: perspectivesData } = useQuery({
    queryKey: ['perspectives', 'fms_bookings'],
    queryFn: async () => {
      const response = await apiCall<PerspectivesIndexResponse>('/api/perspectives/fms_bookings')
  ...
  const { data, isLoading: dataLoading } = useQuery({
    queryKey: ['fms_bookings', queryParams],
    queryFn: async () => {
      const call = await apiCall<{ items: FmsBookingRow[]; total: number; totalPages?: number }>(
        `/api/fms_booking/bookings?${queryParams}`
      )
      if (!call.ok) throw new Error('Failed to load bookings')
  ...
  const tableData = useMemo(() => {
    return (data?.items ?? []).map((booking) => {
      const camelCaseObject: Record<string, any> = { id: booking.id }

      Object.keys(booking).forEach((key) => {
        if (key === 'id') return

        const camelKey = key.replace(/_([a-z])/g, (_, letter) => letter.toUpperCase())
        const value = booking[key as keyof FmsBookingRow]
  ...
  const handleCreateBooking = useCallback(() => {
    router.push('/backend/fms-bookings/new')
  }, [router])
  ...
  useEventHandlers(
    {
      [TableEvents.CELL_EDIT_SAVE]: async (payload: CellEditSaveEvent) => {
        ...
        try {
          const response = await apiCall<{ error?: string }>(
            `/api/fms_booking/bookings/${payload.id}`,
            {
              method: 'PUT',
      ...
      [TableEvents.PERSPECTIVE_SAVE]: async (payload: PerspectiveSaveEvent) => {
        ...
        const response = await apiCall('/api/perspectives/fms_bookings', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        if (response.ok) {
          ...
          queryClient.invalidateQueries({ queryKey: ['perspectives', 'fms_bookings'] })
        } else {
      ...
      [TableEvents.PERSPECTIVE_DELETE]: async (payload: PerspectiveDeleteEvent) => {
        const response = await apiCall(`/api/perspectives/fms_bookings/${payload.id}`, {
          method: 'DELETE',
        })
        if (response.ok) {
