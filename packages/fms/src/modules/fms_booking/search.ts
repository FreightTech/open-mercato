/**
 * FMS Booking Module - Search Configuration
 * CRITICAL: Enables Cmd+K global search for bookings
 */
...
export const searchConfig: SearchModuleConfig = {
  entities: [
    {
      entityId: E.fms_booking.booking,
      enabled: true,
      ...
      buildSource: async (ctx: SearchBuildContext) => {
        ...
        if (record.booking_number) {
          lines.push(`Booking: ${record.booking_number}`)
        }
        ...
        return {
          text: lines,
          presenter: {
            title: pickString(record.booking_number) ?? 'Booking',
            subtitle: [
      ...
      formatResult: async (ctx: SearchBuildContext) => {
        ...
        return {
          title: pickString(ctx.record.booking_number) ?? 'Booking',
          subtitle: [
      ...
      resolveUrl: async (ctx: SearchBuildContext) => {
        return `/backend/fms-bookings/${ctx.record.id}`
      },
