    ...
    for (const item of files) {
      ...
      try {
        ...
        if (documentId) {
          ...
          // If extraction is enabled and onExtract is provided, extract the document
          if (enableExtraction && onExtract) {
            setFiles((prev) =>
              prev.map((f) =>
                f.id === item.id ? { ...f, status: 'extracting', documentId } : f
              )
            )
          ...
          // Mark as success with extraction result
          setFiles((prev) =>
            prev.map((f) =>
              f.id === item.id
                  ...
                  >
                    <div className="flex items-center gap-3 p-2">
                      <div className="flex-shrink-0">
                        ...
                        {item.status === 'success' && !hasExtractionError && (
