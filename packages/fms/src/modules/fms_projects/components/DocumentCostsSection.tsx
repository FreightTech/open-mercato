...
export function DocumentCostsSection({ projectId }: { projectId: string }) {
  const { data } = useQuery({
    queryKey: ['project-matched-documents', projectId],
    queryFn: async (): Promise<MatchedDocumentsResponse | null> => {
      const response = await apiCall<MatchedDocumentsResponse>(
        `/api/fms_projects/projects/${projectId}/matched-documents`
      )
