...
import { updateCarrierSchema } from '../../../data/validators'
import { CommandBus } from '@open-mercato/shared/lib/commands/command-bus'
import type { CommandRuntimeContext } from '@open-mercato/shared/lib/commands'
// Import to register commands
import '../../../commands/carriers'

const updateBodySchema = updateCarrierSchema.omit({ updatedBy: true })
...
export async function GET(req: Request, ctx: { params?: { id?: string } }) {
  ...
  if (!parse.success) return NextResponse.json({ error: 'Invalid carrier id' }, { status: 400 })
  ...
  const carrier = await em.findOne(FmsCarrier, filters)
...
export async function PUT(req: Request, routeCtx: { params?: { id?: string } }) {
  ...
  try {
    const { result } = await bus.execute('fms_products.carriers.update', {
      input: {
        id: parse.data.id,
        name: validation.data.name,
        carrierType: validation.data.carrierType,
    ...
    // Fetch updated carrier for response
    const em = container.resolve('em') as EntityManager
    const carrier = await em.findOne(FmsCarrier, { id: (result as { id: string }).id })

    return NextResponse.json(carrier)
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Failed to update carrier'
...
export async function DELETE(req: Request, routeCtx: { params?: { id?: string } }) {
  ...
  try {
    await bus.execute('fms_products.carriers.delete', {
