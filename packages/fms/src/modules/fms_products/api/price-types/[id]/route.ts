...
import { updatePriceTypeSchema } from '../../../data/validators'
import { CommandBus } from '@open-mercato/shared/lib/commands/command-bus'
import type { CommandRuntimeContext } from '@open-mercato/shared/lib/commands'
// Import to register commands
import '../../../commands/price-types'

const updateBodySchema = updatePriceTypeSchema.omit({ updatedBy: true })
...
export async function GET(req: Request, ctx: { params?: { id?: string } }) {
  ...
  if (!parse.success) return NextResponse.json({ error: 'Invalid price type id' }, { status: 400 })
  ...
  const priceType = await em.findOne(FmsPriceType, filters)

  if (!priceType) return NextResponse.json({ error: 'Price type not found' }, { status: 404 })
...
export async function PUT(req: Request, routeCtx: { params?: { id?: string } }) {
  ...
  try {
    const { result } = await bus.execute('fms_products.price_types.update', {
      input: {
        id: parse.data.id,
    ...
    // Fetch updated price type for response
    const em = container.resolve('em') as EntityManager
    const priceType = await em.findOne(FmsPriceType, { id: (result as { id: string }).id })

    return NextResponse.json(priceType)
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Failed to update price type'
...
export async function DELETE(req: Request, routeCtx: { params?: { id?: string } }) {
  ...
  try {
    await bus.execute('fms_products.price_types.delete', {
    ...
    const message = err instanceof Error ? err.message : 'Failed to delete price type'
    const status = message.includes('not found') ? 404 : 400
    return NextResponse.json({ error: message }, { status })
  }
}
