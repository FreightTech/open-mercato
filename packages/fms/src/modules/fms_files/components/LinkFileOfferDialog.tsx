...
export function LinkFileOfferDialog({ open, onOpenChange, fileId, onOfferLinked }: LinkFileOfferDialogProps) {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiCall<{ items: any[] }>('/api/fms_offers/offers?limit=50')
  ...
  const linkOfferMutation = useMutation({
    mutationFn: async (offerId: string) => {
      const response = await apiCall(`/api/fms_files/files/${fileId}/link-offer`, {
        method: 'POST',
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>Link Offer to File</DialogTitle>
          <DialogDescription>
            Select an offer to link to this file. Offer lines will be imported as file lines.
          </DialogDescription>
        </DialogHeader>
