...
function buildShipmentPresenter(
  ...
  const title = internalRef ?? bookingNumber ?? (record.id as string | undefined) ?? 'Shipment'
  ...
  entities: [
    {
      ...
      buildSource: async (ctx: SearchBuildContext): Promise<SearchIndexSource | null> => {
        ...
        const presenter = buildShipmentPresenter(record)
      ...
      formatResult: async (ctx: SearchBuildContext): Promise<SearchResultPresenter | null> => {
        return buildShipmentPresenter(ctx.record)
      },

      resolveUrl: async (ctx: SearchBuildContext): Promise<string | null> => {
        const id = ctx.record.id as string | undefined
        if (!id) return null
        return `/backend/shipments?id=${encodeURIComponent(id)}`
      },
