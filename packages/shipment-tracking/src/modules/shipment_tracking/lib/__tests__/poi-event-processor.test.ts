...
describe('poi-event-processor', () => {
  ...
  describe('createTrackingEventFromPoi', () => {
    ...
    it('should set vessel name from POI event', () => {
      ...
      expect(trackingEvent.vesselName).toBe('WILSON FINNFJORD')
    })
  ...
  describe('with real NATS messages', () => {
    describe('WAYPOINT_REACHED events', () => {
      it('should parse WILSON FINNFJORD waypoint event', () => {
        const event = poiFixtures.waypointReached[0]

        expect(event.type).toBe('WAYPOINT_REACHED')
        expect(event.mmsi).toBe(314398000)
        expect(event.shipId).toBe(9491769)
        expect(event.shipName).toBe('WILSON FINNFJORD')
      ...
      it('should parse COSMIC TIGER waypoint event', () => {
        const event = poiFixtures.waypointReached[1]

        expect(event.type).toBe('WAYPOINT_REACHED')
        expect(event.mmsi).toBe(352004201)
        expect(event.shipName).toBe('COSMIC TIGER')
    ...
    describe('PORT_ARRIVAL events', () => {
      it('should parse KAKO port arrival', () => {
        const event = poiFixtures.portArrival[0]

        expect(event.type).toBe('PORT_ARRIVAL')
        expect(event.mmsi).toBe(431025922)
        expect(event.shipName).toBe('KAKO')
        expect(event.arrivedPoiCode).toBe('N035E139-03434')
        expect(event.sequenceOrder).toBe(2)
      })

      it('should parse GRANDE SENEGAL port arrival with very close distance', () => {
        const event = poiFixtures.portArrival[1]

        expect(event.type).toBe('PORT_ARRIVAL')
        expect(event.mmsi).toBe(247285500)
        expect(event.shipName).toBe('GRANDE SENEGAL')
    ...
    describe('TERMINAL_ARRIVAL events', () => {
      it('should parse KAKO terminal arrival (same vessel as port arrival)', () => {
        const event = poiFixtures.terminalArrival[0]
      ...
      it('should parse ALS CLIVIA terminal arrival in Sydney', () => {
        const event = poiFixtures.terminalArrival[2]

        expect(event.type).toBe('TERMINAL_ARRIVAL')
        expect(event.shipName).toBe('ALS CLIVIA')
    ...
    describe('proximity arrival events', () => {
      it('should parse MARIBO MAERSK port proximity arrival', () => {
        const event = poiFixtures.portProximityArrival[0]

        expect(event.type).toBe('PORT_PROXIMITY_ARRIVAL')
        expect(event.mmsi).toBe(219019094)
        expect(event.shipName).toBe('MARIBO MAERSK')
        expect(event.distanceToPoiMeters).toBeCloseTo(136.13, 1)
      })

      it('should parse NYK REMUS terminal proximity arrival', () => {
        const event = poiFixtures.terminalProximityArrival[0]

        expect(event.type).toBe('TERMINAL_PROXIMITY_ARRIVAL')
        expect(event.mmsi).toBe(357175000)
        expect(event.shipName).toBe('NYK REMUS')
  ...
  describe('POI code format', () => {
    ...
    it('should use S for southern latitudes', () => {
      const event = poiFixtures.terminalArrival[2] // ALS CLIVIA in Sydney
      const code = getPoiCode(event)
    ...
    it('should use W for western longitudes', () => {
      const event = poiFixtures.terminalProximityArrival[0] // NYK REMUS at -118...
      const code = getPoiCode(event)
