...
export async function resolveWidgetScope(
  ...
  if (!auth) {
    throw new CrudHttpError(401, { error: translate('frc_rfqs.errors.unauthorized', 'Unauthorized') })
  }
  ...
  if (!tenantId) {
    throw new CrudHttpError(400, { error: translate('frc_rfqs.errors.tenant_required', 'Tenant context is required') })
  }
  ...
  if (organizationIds !== null && organizationIds.length === 0) {
    throw new CrudHttpError(400, { error: translate('frc_rfqs.errors.organization_required', 'Organization context is required') })
  }
