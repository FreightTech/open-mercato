...
import { updateProjectAirRoutingSchema } from '../../../../../data/validators'
...
function formatDateForResponse(date: Date | string | null | undefined): string | null {
  if (!date) return null
...
export async function GET(req: Request, ctx: { params?: Promise<{ id?: string; routingId?: string }> }) {
  ...
  const routing = await em.findOne(FrcProjectAirRouting, {
  ...
  // Count connected consoles
  const connectedConsolesCount = await em.count(FrcConsole, {
    projectAirRoutingId: routing.id,
    deletedAt: null,
  })
  ...
  return NextResponse.json({
    ...
    originAirport,
    destinationAirport,
    departureDate: formatDateForResponse(routing.departureDate),
    departureTime: routing.departureTime ?? null,
    arrivalDate: formatDateForResponse(routing.arrivalDate),
    arrivalTime: routing.arrivalTime ?? null,
    connectedConsolesCount,
  })
}

export async function PUT(req: Request, ctx: { params?: Promise<{ id?: string; routingId?: string }> }) {
  const auth = await getAuthFromRequest(req)
  if (!auth) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  ...
  const validation = updateProjectAirRoutingSchema.safeParse(body)
  ...
  if (shouldRegenerateName) {
    const airportIds = [originAirportId, destinationAirportId].filter((id): id is string => Boolean(id))
...
export async function DELETE(req: Request, ctx: { params?: Promise<{ id?: string; routingId?: string }> }) {
  const auth = await getAuthFromRequest(req)
  if (!auth) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  ...
  // Find and soft-delete connected consoles and their cargo
  ...
  return NextResponse.json({ success: true, deletedConsolesCount })
}
