...
export async function resolveWidgetScope(
  ...
  if (!auth) {
    throw new CrudHttpError(401, { error: translate('frc_trucks.errors.unauthorized', 'Unauthorized') })
  }
  ...
  if (organizationIds !== null && organizationIds.length === 0) {
    throw new CrudHttpError(400, { error: translate('frc_trucks.errors.organization_required', 'Organization context is required') })
  }
