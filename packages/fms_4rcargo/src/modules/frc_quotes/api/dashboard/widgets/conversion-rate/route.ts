...
export async function GET(req: Request) {
  ...
  try {
    ...
    // Build base filter for completed quotes (booked, rejected, expired)
    const baseWhere: FilterQuery<FrcQuote> = {
      tenantId,
      deletedAt: null,
    ...
    // Get quotes
    const quotes = await em.find(FrcQuote, baseWhere)
    ...
    if (showComparison) {
      ...
      const prevWhere: FilterQuery<FrcQuote> = {
        tenantId,
        deletedAt: null,
        status: { $in: ['booked', 'rejected', 'expired'] },
      ...
      const prevQuotes = await em.find(FrcQuote, prevWhere)
      let prevBooked = 0
      let prevRejected = 0
      let prevExpired = 0
    ...
    console.error('frc_quotes.widgets.conversionRate failed', err)
    return NextResponse.json(
      { error: translate('frc_quotes.widgets.conversionRate.error', 'Failed to load quote conversion data') },
      { status: 500 },
    )
  }
}
...
export const openApi: OpenApiRouteDoc = {
  tag: '4R Cargo Quotes',
  summary: 'Quote conversion rate widget',
  methods: {
    GET: {
      summary: 'Fetch quote conversion rate metrics',
      description: 'Returns conversion rate and breakdown of quotes by final status within the scoped tenant/organization.',
      query: querySchema,
      responses: [
        {
          status: 200,
