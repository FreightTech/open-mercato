...
async function loadDeparturesData(settings: UpcomingDeparturesSettings): Promise<UpcomingDeparturesData> {
  ...
  const call = await apiCall<UpcomingDeparturesData>(`/api/frc_quotes/dashboard/widgets/upcoming-departures?${params}`)
  if (!call.ok) {
    const message =
...
const UpcomingDeparturesWidget: React.FC<DashboardWidgetComponentProps<UpcomingDeparturesSettings>> = ({
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      setError(t('frc_quotes.widgets.upcomingDepartures.error', 'Failed to load data'))
    } finally {
      setLoading(false)
    ...
    return (
      <div className="space-y-4 text-sm">
        <div className="space-y-1.5">
          <label className="text-sm font-medium">{t('frc_quotes.widgets.upcomingDepartures.settings.daysAhead', 'Days Ahead')}</label>
          <select
        ...
        <div className="space-y-1.5">
          <label className="text-sm font-medium">{t('frc_quotes.widgets.upcomingDepartures.settings.maxItems', 'Max Items')}</label>
          <input
            type="number"
            min={1}
            max={10}
    ...
    <div className="space-y-3">
      {(loading || !data) ? (
        ...
        <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
          ...
          <p className="text-sm">{t('frc_quotes.widgets.upcomingDepartures.noDepartures', 'No upcoming departures')}</p>
        </div>
      ) : (
        <>
          {data.departures.map((departure) => (
            <Link
              ...
              href={`/backend/frc-quotes?id=${departure.quoteId}`}
