...
async function loadConversionData(settings: ConversionRateSettings): Promise<ConversionData> {
  ...
  const call = await apiCall<ConversionData>(`/api/frc_quotes/dashboard/widgets/conversion-rate?${params}`)
  if (!call.ok) {
    const message =
...
const ConversionRateWidget: React.FC<DashboardWidgetComponentProps<ConversionRateSettings>> = ({
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to load quote conversion rate widget data', err)
      setError(t('frc_quotes.widgets.conversionRate.error', 'Failed to load data'))
    } finally {
      setLoading(false)
    ...
    return (
      <div className="space-y-4 text-sm">
        <div className="space-y-1.5">
          <label className="text-sm font-medium">{t('frc_quotes.widgets.conversionRate.settings.dateRange', 'Date Range')}</label>
          <select
        ...
        <div className="space-y-1.5">
          <label className="flex items-center gap-2 text-sm">
            ...
            />
            {t('frc_quotes.widgets.conversionRate.settings.showComparison', 'Show comparison to previous period')}
          </label>
        </div>
      </div>
    )
      ...
      {(loading || !data) ? (
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-2 text-sm">
            <div className="flex items-center gap-2 rounded-md bg-green-50 p-2">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">{t('frc_quotes.widgets.conversionRate.booked', 'Booked')}</div>
              </div>
            </div>
            <div className="flex items-center gap-2 rounded-md bg-red-50 p-2">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">{t('frc_quotes.widgets.conversionRate.rejected', 'Rejected')}</div>
              </div>
            </div>
            <div className="flex items-center gap-2 rounded-md bg-gray-50 p-2">
              ...
              <div>
                ...
                <div className="text-xs text-muted-foreground">{t('frc_quotes.widgets.conversionRate.expired', 'Expired')}</div>
              </div>
            </div>
          </div>
          ...
          <div className="text-xs text-muted-foreground text-center">
            {t('frc_quotes.widgets.conversionRate.totalQuotes', 'Total completed quotes')}: {data.total}
          </div>
        </>
      )}
    </div>
