...
import { createAirportSchema, airportFilterSchema } from '../../data/validators'

export const metadata = {
  GET: { requireAuth: true, requireFeatures: ['frc_airports.view'] },
  POST: { requireAuth: true, requireFeatures: ['frc_airports.manage'] },
}
...
export async function GET(request: NextRequest) {
  ...
  const query = {
    ...
    sortField: url.searchParams.get('sortField') || 'code',
    sortDir: url.searchParams.get('sortDir') || 'asc',
  }

  const parse = airportFilterSchema.safeParse(query)
  if (!parse.success) {
  ...
  if (parse.data.code) {
    filters.code = { $ilike: `%${escapeLikePattern(parse.data.code)}%` }
  }

  if (parse.data.country) {
    filters.country = { $ilike: `%${escapeLikePattern(parse.data.country)}%` }
  }
  ...
  const sortField = sortFieldMap[parse.data.sortField] || 'code'
  const sortDir = parse.data.sortDir

  const [items, total] = await em.findAndCount(FrcAirport, filters, {
...
export async function POST(request: NextRequest) {
  ...
  const parse = createAirportSchema.safeParse(body)
  ...
  const now = new Date()
  const airport = em.create(FrcAirport, {
    organizationId: organizationId as string,
    tenantId: tenantId as string,
    code: parse.data.code,
    longCode: parse.data.longCode,
  ...
  await em.persistAndFlush(airport)
