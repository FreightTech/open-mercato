...
describe('KSeF crypto', () => {
  ...
  describe('encryptTokenForKsef', () => {
    ...
    it('produces a base64-encoded encrypted payload', () => {
      ...
      // 4-byte header + 256-byte wrapped key + 16-byte IV + encrypted token
      expect(decoded.length).toBeGreaterThan(4 + 256 + 16)

      // Header stores length as [0, 0, 0, len & 0xFF] — 256 wraps to 0
      const headerBytes = decoded.subarray(0, 4)
      expect(headerBytes[0]).toBe(0)
      expect(headerBytes[1]).toBe(0)
      expect(headerBytes[2]).toBe(0)

      // The wrapped RSA key starts at offset 4 and is 256 bytes (2048-bit RSA)
      const wrappedKey = decoded.subarray(4, 4 + 256)
      expect(wrappedKey.length).toBe(256)
    })
  })
