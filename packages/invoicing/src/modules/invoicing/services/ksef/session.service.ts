...
export class KsefSessionService {
  ...
  ): Promise<InvoicingKsefSession | null> {
    return em.findOne(InvoicingKsefSession, {
  ...
  async getSessionById(em: EntityManager, sessionId: string): Promise<InvoicingKsefSession | null> {
    return em.findOne(InvoicingKsefSession, { id: sessionId })
  }

  async requireActiveSession(
    em: EntityManager,
    params: GetActiveSessionParams
  ): Promise<InvoicingKsefSession> {
  ...
  async closeStaleActiveSessions(
    ...
    const staleSessions = await em.find(InvoicingKsefSession, {
      tenantId: params.tenantId,
      organizationId: params.organizationId,
      sessionStatus: 'active',
      startedAt: { $lt: cutoff },
