...
import { InvoicingInvoice, InvoicingKsefSession, InvoicingSettings } from '../data/entities'
...
export const UPO_DOWNLOAD_QUEUE_NAME = 'invoicing-ksef-upo-download'

export const metadata: WorkerMeta = {
  queue: UPO_DOWNLOAD_QUEUE_NAME,
  concurrency: 3,
  id: 'invoicing-ksef-upo-download',
}
...
const logger = createInvoicingLogger('invoicing.ksef_upo_download')
...
export default async function handle(
  ...
  try {
    ...
    for (const invoice of invoices) {
      await emitInvoicingEvent('invoicing.ksef.upo_downloaded', {
        id: invoice.id,
