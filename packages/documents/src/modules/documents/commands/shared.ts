...
import type { DocumentSnapshot } from '../data/snapshots'
...
export async function loadDocumentSnapshot(
  ...
  const document = await em.findOne(Document, { id: documentId, deletedAt: null })
...
export async function applyDocumentSnapshot(
  ...
  let document = await em.findOne(Document, { id: snapshot.id })
