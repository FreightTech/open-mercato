...
import { z } from 'zod'
import { Document, DocumentCategory } from '../data/entities'
import type { DocumentSnapshot, DocumentUndoPayload } from '../data/snapshots'
import {
  ensureTenantScope,
...
const documentsUpdatedCounter = meter.createCounter('documents.updated', {
  description: 'Number of documents updated',
  unit: '1',
})

const documentsDeletedCounter = meter.createCounter('documents.deleted', {
...
const createDocumentCommand: CommandHandler<CreateDocumentInput, { id: string }> = {
  ...
  async execute(rawInput, ctx) {
    ...
    await emitCrudSideEffects({
      ...
      indexer: { entityType: 'documents:document' },
    })
...
const updateDocumentCommand: CommandHandler<UpdateDocumentInput, { id: string }> = {
  ...
  async execute(rawInput, ctx) {
    ...
    const document = await em.findOne(Document, { id: input.id, deletedAt: null })
  ...
  buildLog: async ({ ctx, snapshots, result }) => {
    const before = snapshots.before as DocumentSnapshot | undefined
  ...
  undo: async ({ logEntry, ctx }) => {
    ...
    const de = ctx.container.resolve('dataEngine') as DataEngine
    const document = await em.findOne(Document, { id: before.id })
    if (document) {
...
const deleteDocumentCommand: CommandHandler<{ id?: string; body?: Record<string, unknown>; query?: Record<string, unknown> }, { id: string }> = {
  ...
  async execute(input, ctx) {
    ...
    const document = await em.findOne(Document, { id, deletedAt: null })
    const record = assertRecordFound(document, 'Document not found')
    ensureTenantScope(ctx, record.tenantId)
    ensureOrganizationScope(ctx, record.organizationId)
...
const updateDocumentDataCommand: CommandHandler<UpdateDocumentDataInput, { id: string }> = {
  id: 'documents.documents.updateDocumentData',
  async execute(rawInput, ctx) {
    ...
    if (linkingFieldsChanged && !record.relatedEntityId) {
      ...
      await eventBus.emitEvent('documents.document.identifiers_updated', payload, { persistent: true })
    }
