...
function resolveSchemasDir(): string {
  ...
  const possiblePaths = [
    ...
    path.join(process.cwd(), 'packages/documents/src/modules/documents/data/schemas'),
    ...
    path.join(path.dirname(fileURLToPath(import.meta.url)), '..', '..', '..', '..', 'src', 'modules', 'documents', 'data', 'schemas'),
  ]
