...
export default function TableViewerPage({ params }: { params?: { id?: string } }) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['table-data', definitionId],
    queryFn: async () => {
      ...
      if (!result.ok || !result.result) {
        ...
        throw new Error(typeof errorPayload?.error === 'string' ? errorPayload.error : 'Failed to load table data')
      }
      return result.result
    },
    enabled: !!definitionId,
    refetchInterval: isSaving ? false : 5000,
  })
      ...
      [TableEvents.CELL_EDIT_SAVE]: async (payload: CellEditSaveEvent) => {
        ...
        try {
          ...
          if (result.ok) {
            ...
            await queryClient.invalidateQueries({ queryKey: ['table-data', definitionId] })
          } else {
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" onClick={() => queryClient.invalidateQueries({ queryKey: ['table-data', definitionId] })}>
