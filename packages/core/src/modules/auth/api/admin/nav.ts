...
import type { BackendChromeNavGroup } from '@open-mercato/shared/modules/navigation/backendChrome'
...
export async function GET(req: Request) {
  ...
  const brandFilteredGroups = applyBrandFiltering(
    payload.groups.map((group) => ({ ...group, id: group.id ?? '' })),
    brandConfig,
  ) as BackendChromeNavGroup[]
  const brandFilteredPayload = { ...payload, groups: brandFilteredGroups }
  ...
  try {
    if (cache?.set) {
      ...
      await cache.set(cacheKey, brandFilteredPayload, { tags })
    }
  } catch {
    // ignore cache write failures
  }

  return NextResponse.json(brandFilteredPayload)
}
