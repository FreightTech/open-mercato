...
import { Globe, Mail, Database, Radio, Shuffle, Code, Send, Filter, Reply, Minus, Plus, Play, Power, Trash2, MoreHorizontal, CheckCircle2 } from 'lucide-react'
import { useConnectCtx } from '../AutomationCanvas'

const CARD_SIZE = 76
const ACTION_ICONS: Record<string, typeof Globe> = {
  'action.http_request': Globe, 'action.send_email': Mail, 'action.update_entity': Database,
  'action.emit_event': Radio, 'action.transform': Shuffle, 'action.code': Code,
  'action.filter': Filter, 'action.respond_webhook': Reply, 'utility.noop': Minus,
}

interface NodeProps { id: string; data: Record<string, unknown>; selected?: boolean }

function ActionNodeComponent({ data, selected, id }: NodeProps) {
  const [hovered, setHovered] = useState(false)
  const connectCtx = useConnectCtx()
  const isDraggingFromThis = connectCtx?.draggingSourceId === id
  const Icon = ACTION_ICONS[data.nodeType as string] ?? Send
