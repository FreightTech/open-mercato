...
import { Timer, Variable, Minus, Plus, Play, Power, Trash2, MoreHorizontal, CheckCircle2 } from 'lucide-react'
import { useConnectCtx } from '../AutomationCanvas'

const CARD_SIZE = 76
const UTILITY_ICONS: Record<string, typeof Timer> = {
  'action.delay': Timer, 'action.set_variable': Variable, 'utility.noop': Minus,
}

interface NodeProps { id: string; data: Record<string, unknown>; selected?: boolean }

function UtilityNodeComponent({ data, selected, id }: NodeProps) {
  const [hovered, setHovered] = useState(false)
  const connectCtx = useConnectCtx()
  const isDraggingFromThis = connectCtx?.draggingSourceId === id
  const Icon = UTILITY_ICONS[data.nodeType as string] ?? Timer
