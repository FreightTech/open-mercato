...
export function getAppBaseUrl(req: Request): string {
  // Prefer the request host to preserve brand domain on redirects across
  // different branded/custom domains; fall back to configured env URLs only
  // when no host header is available.
  if (req.headers.get('host') || req.headers.get('x-forwarded-host')) {
